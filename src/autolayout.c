...
static bui_widget_t *_depth_first_next(bui_widget_t *widget)
{
...
static uint32_t _padding_before(bui_widget_t *widget, bui_axis_t axis)
{
...
static uint32_t _padding_after(bui_widget_t *widget, bui_axis_t axis)
{
    if (!(widget->flags & BUI_WIDGET_FLAG_PADDED))
        return 0;
...
static uint32_t _padding_sum(bui_widget_t *widget, bui_axis_t axis)
{
...
static uint32_t _content_size(bui_widget_t *widget, bui_axis_t axis)
{
    return _sub_or_zero(widget->computed_size[axis], _padding_sum(widget, axis));
}

static uint32_t _content_origin(bui_widget_t *widget, bui_axis_t axis)
{
    uint32_t origin = axis == BUI_AXIS_X ? widget->computed_area.x : widget->computed_area.y;
    return origin + _padding_before(widget, axis);
}

static uint32_t _scrolled_origin(bui_widget_t *widget, bui_axis_t axis)
{
...
static void _solve_standalone_sizes(bui_wctx_t *wctx, bui_axis_t axis)
{
    for (bui_widget_t *w = wctx->widget_tree; w != NULL; w = _depth_first_next(w)) {
...
static void _solve_upwards_dependent_sizes(bui_wctx_t *wctx, bui_axis_t axis)
{
    for (bui_widget_t *w = wctx->widget_tree; w != NULL; w = _depth_first_next(w)) {
        if (w->semantic_size[axis].type != BUI_WIDGET_SIZE_TYPE_PERCENTAGE)
            continue;

        bui_widget_t *parent = w->parent;
        if (parent != NULL && parent->layout_axis == axis) {
            uint32_t children_count = 0;
            uint32_t fixed_size = 0;
            uint32_t total_percentage = 0;
            for (bui_widget_t *c = parent->first_child; c != NULL; c = c->next_sibling) {
                children_count++;
            ...
            uint32_t available_size = _sub_or_zero(
...
static void _solve_downwards_dependent_size(bui_widget_t *w, bui_axis_t axis)
{
    if (w == NULL)
        return;

    for (bui_widget_t *c = w->first_child; c != NULL; c = c->next_sibling)
...
static void _solve_downwards_dependent_sizes(bui_wctx_t *wctx, bui_axis_t axis)
{
    _solve_downwards_dependent_size(wctx->widget_tree, axis);
}

static void _solve_violations(bui_wctx_t *wctx, bui_axis_t axis)
{
    for (bui_widget_t *w = wctx->widget_tree; w != NULL; w = _depth_first_next(w)) {
...
static void _compute_areas(bui_wctx_t *wctx, bui_axis_t axis)
{
    for (bui_widget_t *w = wctx->widget_tree; w != NULL; w = _depth_first_next(w)) {
        uint32_t cursor = (w->flags & BUI_WIDGET_FLAG_SCROLLABLE)
                              ? _scrolled_origin(w, axis)
