...
gfx_context_t gfx_init_screen(void)
{
    /* Standalone Linux libui creates SDL windows via ui_new_window().
       This fallback context is useful for tests and off-screen drawing. */
    gfx_context_t ctx = {
        .width = 0,
        .height = 0,
        .pitch = 0,
...
void gfx_end_frame(gfx_context_t *ctx)
{
    ...
    if (ctx->native_renderer && ctx->native_texture && ctx->backbuffer) {
        SDL_Renderer *renderer = (SDL_Renderer *) ctx->native_renderer;
        SDL_Texture *texture = (SDL_Texture *) ctx->native_texture;
        SDL_UpdateTexture(texture, NULL, ctx->backbuffer, (int) (ctx->width * sizeof(uint32_t)));
    ...
    if (ctx->target_fps != 0) {
        ...
        if (ctx->last_frame_ticks < frame_budget)
            SDL_Delay((Uint32) (frame_budget - ctx->last_frame_ticks));
    }
